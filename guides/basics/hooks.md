...
In a Feathers application generated by the CLI, hooks are being registered in in `.hooks` file in an object in the following format:
...
In the `process-message` hook we are currently just adding the user's `_id` as the `userId` property in the message. We want to show more than the `_id` in the UI, so we'll need to populate more data in the message response. To display a users' details, we need to include extra information in our messages. This is one way to associate data with Feathers.

We create another hook:

- Called `populate-user`
- It's an `after` hook
...
In this chapter we learned how Feathers hooks can be used as middleware for service method calls to validate and manipulate incoming and outgoing data without having to change our service. We now have a complete fully working chat application. Before we [create a frontend for it](./frontend.md) though, let's look at how authentication works with Feathers.
